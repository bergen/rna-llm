...
class SequenceLightningModule(pl.LightningModule):
    ...
    def load_state_dict(self, state_dict, strict=False):
        if not self.hparams.train.predict:
        ...
        else: # we are predicting, and therefore want to load both the backbone and the decoder
            pass
    ...
    def _shared_step(self, batch, batch_idx, prefix="train"):
        ...
        if prefix!="predict":
            ...
            log_on_step = 'eval' in self.hparams and self.hparams.eval.get('log_on_step', False)
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0):
        outputs = self._shared_step(batch, batch_idx, prefix="train")
        loss= outputs["loss"]
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        outputs = self._shared_step(
            batch, batch_idx, prefix=self.predict_loader_names[dataloader_idx]
        )
        return outputs
    ...
    def val_dataloader(self):
        val_loader_names, _, val_loaders, _ = self._eval_dataloaders()
        self.val_loader_names = val_loader_names
        return val_loaders

    def test_dataloader(self):
        _, test_loader_names, _, test_loaders = self._eval_dataloaders()
        self.test_loader_names = test_loader_names
        return test_loaders
    
    def predict_dataloader(self):
        predict_loaders = self.dataset.predict_dataloader(**self.hparams.loader)
        predict_loader_names, predict_loaders = self._eval_dataloaders_names(predict_loaders, "predict")
        self.predict_loader_names = predict_loader_names
...
def train(config):
    ...
    elif config.train.only_validate:
        trainer.validate(model)
    elif config.train.ckpt is not None:
