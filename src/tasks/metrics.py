...
def cross_entropy(logits, y, weights=None, ignore_index=-100):
    if weights is not None:
        weight = torch.Tensor(weights)
        weight = weight.to(logits)
    else:
        weight = None
    logits = logits.view(-1, logits.shape[-1])
    y = y.view(-1)
    return F.cross_entropy(logits, y, weight=weight, ignore_index=ignore_index)
...
def preprocess_logits_and_labels(logits, y):
    logits = logits.view(-1, logits.shape[-1])
    preds = torch.argmax(logits, dim=-1)
...
def true_positives(logits, y, positive_class=1):
    preds, y = preprocess_logits_and_labels(logits, y)
    return torch.sum((preds == positive_class) & (y == positive_class)).float().item()

def false_negatives(logits, y, positive_class=1):
    preds, y = preprocess_logits_and_labels(logits, y)
    return torch.sum((preds != positive_class) & (y == positive_class)).float().item()

def true_negatives(logits, y, positive_class=1):
    preds, y = preprocess_logits_and_labels(logits, y)
    return torch.sum((preds != positive_class) & (y != positive_class)).float().item()

def false_positives(logits, y, positive_class=1):
    preds, y = preprocess_logits_and_labels(logits, y)
    return torch.sum((preds == positive_class) & (y != positive_class)).float().item()
...
#this won't work because it's computed on a batch level, and we don't want to average batch level estimates of recall/specificity
def recall(logits, y, positive_class=1):
    logits = logits.view(-1, logits.shape[-1])
    preds = torch.argmax(logits, dim=-1)
    ...
    true_positives = torch.sum((preds == positive_class) & (y == positive_class)).float()
    false_negatives = torch.sum((preds != positive_class) & (y == positive_class)).float()
    
    return true_positives / (true_positives + false_negatives)

def specificity(logits, y, positive_class=1):
    logits = logits.view(-1, logits.shape[-1])
    preds = torch.argmax(logits, dim=-1)
    ...
    true_negatives = torch.sum((preds != positive_class) & (y != positive_class)).float()
    false_positives = torch.sum((preds == positive_class) & (y != positive_class)).float()
